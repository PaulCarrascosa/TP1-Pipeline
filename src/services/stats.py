...
class StatsService:
    ...
    def get_most_borrowed_books(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {"id": row.id, "title": row.title, "author": row.author, "loan_count": row.loan_count}
            for row in result
        ]
    ...
    def get_most_active_users(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {"id": row.id, "full_name": row.full_name, "email": row.email, "loan_count": row.loan_count}
            for row in result
        ]
    ...
    def get_monthly_loans(self, months: int = 12) -> List[Dict[str, Any]]:
        ...
        return [{"month": month, "loan_count": loan_count} for month, loan_count in result]
